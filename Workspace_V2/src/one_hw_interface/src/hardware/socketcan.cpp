 ...
 */
    ...
    SocketCAN::SocketCAN(const std::string& interface, boost::function<void(const can_frame &)> handler) :
        interface_(interface), reception_handler(std::move(handler)) {
    }
    ...
    bool SocketCAN::open() {
        if (is_open()) {
            ROS_ERROR("Error: CAN socket already open.");
            return false;
        }
        ...
        char name[IFNAMSIZ] = {};  // avoid stringop-truncation
        strncpy(name, interface_.c_str(), std::min(interface_.size(), (size_type)IFNAMSIZ));
